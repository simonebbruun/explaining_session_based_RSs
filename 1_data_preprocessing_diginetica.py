...
data = data.rename(columns={'sessionId': 'session_id', 'itemId': 'item_id', 'timeframe': 'timestamp'})

data = data[['session_id', 'eventdate', 'timestamp', 'item_id']]
...
while (min_session<5) or (min_item<20):
    ...
    item_count = item_count[item_count['count']>=20]
...
sessions = data[['session_id', 'eventdate']].groupby('session_id', as_index=False)['eventdate'].min()
sessions = sessions.sort_values('eventdate').reset_index(drop=True)
...
data_test = data_test.drop('eventdate', axis=1)


# Timestamp as one column.
data_train['timestamp'] = data_train[['eventdate','timestamp']].apply(tuple,axis=1).rank(method='dense').astype(int)
data_train = data_train.drop('eventdate', axis=1)


# Export.
data_train.to_csv('diginetica_train.csv', index=False)
data_test.to_csv('diginetica_test.csv', index=False)
